...
<c:set var="myCartList"  value="${companyMap.companyList}"  />
			...
			<tr style="background:#33ff00" >
			 ...
			 <tr>       
               <form name="frm_order_all_cart">
				      <c:forEach var="item" items="${companyList }" varStatus="cnt">
				       <c:set var="company_id" value="${companyList[cnt.count-1].company_id}" />
				       <c:set var="business_registration_number" value="${companyList[cnt.count-1].business_registration_number}" />


					<td class="goods_image">
					<a href="${contextPath}/goods/goodsDetail.do?goods_id=${item.company_id }">
						<img width="75" alt="" src="${contextPath}/thumbnails.do?goods_id=${item.company_id}&fileName=${item.company_name}"  />
					</a>
					</td>
					<td>
						<h2>
							<a href="${contextPath}/goods/goodsDetail.do?goods_id=${item.company_id }">${item.company_name }</a>
						</h2>
					</td>
					<td class="price"><span>${item.representative_name }원</span></td>
					<td>
					   <strong>
					      <fmt:formatNumber  value="${item.company_id*0.9}" type="number" var="discounted_price" />
				            ${discounted_price}원(10%할인)
				         </strong>
					</td>
					<td>
					   <input type="text" id="business_type" name="business_type" size=3 value="${business_type}"><br>
 
					</td>
					<td>
					   <strong>
					    <fmt:formatNumber  value="${item.company_id*0.9*company_id}" type="number" var="total_sales_price" />
				         ${total_sales_price}원
					</strong> </td>
					<td>
					      <a href="javascript:fn_order_each_goods('${item.company_id }','${item.business_type }','${item.company_name}','${item.representative_name}');">
						...
						<a href="javascript:delete_cart_goods('${company_id}');""> 
						   <img width="75" alt=""
							   src="${contextPath}/resources/image/btn_delete.jpg">
					   </a>
					</td>
			</tr>
				<c:set  var="totalGoodsPrice" value="${totalGoodsPrice+item.company_id*0.9*company_id }" />
